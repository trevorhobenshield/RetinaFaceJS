...
async function main() {
  ...
  try {
    const session = await ort.InferenceSession.create('det_10g.onnx', {
    ...
    const T = img_to_tensor(img, INPUT_DIMS, x => (x - MEAN) / STD);

    let out = await session.run({'input.1': T});
    ...
    console.log('score =', det.flat().pop());
    console.log('bbox =', det.flat().slice(0, 4));
    console.log('kps =', kpss.flat());
  } catch (e) {
    console.log(e);
  ...
  console.log(`inference took ${(end - start) / 1000} seconds`);
}
